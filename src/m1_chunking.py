...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for parent in parents:
        ...
        for sentence in parent_sentences:
            if len(current_child_text) + len(sentence) + (1 if current_child_text else 0) > child_size:
                ...
                if len(sentence) > child_size:
                    ...
                    sub_child = ""
                    for word in words:
                        if len(sub_child) + len(word) + (1 if sub_child else 0) > child_size:
                            if sub_child:
                                children.append(Chunk(
