...
# Helper for client creation compatible with Groq / OpenAI
    ...
    if client and model:
        try:
            ...
            resp = client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": 'Trích xuất metadata từ đoạn văn. Trả về JSON hợp lệ có dạng: {"topic": "...", "entities": ["..."], "category": "policy|hr|it|finance", "language": "vi|en"}. Không viết gì thêm ngoài JSON.'},
                    {"role": "user", "content": text},
                ],
                max_tokens=150,
                **kwargs
...
Không viết gì thêm ngoài JSON."""},
    ...
    context_line = f"{prefix}Nội dung về tài liệu {source}."
    return {
        "summary": summary,
        "questions": questions,
